...
def process_file(file, location_coords):
    ...
    # Extract the temperature variable (t) instead of cloud cover
    temperature = ds.variables['t'][:]  # Shape: (time, pressure_levels, lat, lon)
    ...
    # Extract temperature and geopotential height time series for the location
    temperature_profile = temperature[:, :, lat_idx, lon_idx] - 273.15  # Convert K to Celsius
    geopotential_profile = (geopotential_height[:, :, lat_idx, lon_idx] / 9.81) / 1000  # Convert z to z/g (km)

    # Close the NetCDF file
    ds.close()

    return temperature_profile, geopotential_profile, time_values, pressure_levels

# Process files for Athens and Thessaloniki
athens_temperature, athens_geopotential, athens_time_values, pressure_levels = process_file(athens_file, athens_coords)
thessaloniki_temperature, thessaloniki_geopotential, thessaloniki_time_values, _ = process_file(thessaloniki_file, thessaloniki_coords)
...
# Function to calculate the average vertical profile of temperature
def calculate_average_profile(temperature, indices):
    return np.mean(temperature[indices, :], axis=0) if indices else None

# Plot all four seasons in one figure
fig, axs = plt.subplots(2, 2, figsize=(15, 12))

# Function to plot average temperature profiles for each season
def plot_season_profiles(ax, season, athens_time_indices, thessaloniki_time_indices, athens_temperature, athens_geopotential, thessaloniki_temperature, thessaloniki_geopotential):
    for time_label, indices in athens_time_indices.items():
        avg_profile = calculate_average_profile(athens_temperature, indices)
    ...
    for time_label, indices in thessaloniki_time_indices.items():
        avg_profile = calculate_average_profile(thessaloniki_temperature, indices)
        if avg_profile is not None:
    ...
    ax.set_title(f'Average Vertical Profile of Temperature in {season}')
    ax.legend()
    ax.set_xlim(-5, 30)  # Zoom in on x-axis to show typical temperature range (°C)
    ax.set_ylim(0, 3)  # Zoom in on y-axis to show height range (0 to 3 km)

# Plot for Winter
plot_season_profiles(axs[0, 0], 'Winter', athens_winter_time_indices, thessaloniki_winter_time_indices, athens_temperature, athens_geopotential, thessaloniki_temperature, thessaloniki_geopotential)

# Plot for Spring
plot_season_profiles(axs[0, 1], 'Spring', athens_spring_time_indices, thessaloniki_spring_time_indices, athens_temperature, athens_geopotential, thessaloniki_temperature, thessaloniki_geopotential)

# Plot for Summer
plot_season_profiles(axs[1, 0], 'Summer', athens_summer_time_indices, thessaloniki_summer_time_indices, athens_temperature, athens_geopotential, thessaloniki_temperature, thessaloniki_geopotential)

# Plot for Autumn
plot_season_profiles(axs[1, 1], 'Autumn', athens_autumn_time_indices, thessaloniki_autumn_time_indices, athens_temperature, athens_geopotential, thessaloniki_temperature, thessaloniki_geopotential)
