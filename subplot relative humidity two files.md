...
athens_file = 'athinatwo.nc'  # File containing relative humidity data for Athens
thessaloniki_file = 'thessalonikitwo.nc'  # File containing relative humidity data for Thessaloniki
...
def process_file(file, location_coords):
    ...
    # Extract the relative humidity variable
    relative_humidity = ds.variables['r'][:]  # Shape: (time, pressure_levels, lat, lon)
    ...
    # Extract relative humidity and geopotential height time series for the location
    humidity_profile = relative_humidity[:, :, lat_idx, lon_idx]  # Shape: (time, pressure_levels)
    geopotential_profile = (geopotential_height[:, :, lat_idx, lon_idx] / 9.81) / 1000  # Convert z to z/g (km)

    # Close the NetCDF file
    ds.close()

    return humidity_profile, geopotential_profile, time_values, pressure_levels

# Process files for Athens and Thessaloniki
athens_humidity, athens_geopotential, athens_time_values, pressure_levels = process_file(athens_file, athens_coords)
thessaloniki_humidity, thessaloniki_geopotential, thessaloniki_time_values, _ = process_file(thessaloniki_file, thessaloniki_coords)
...
# Function to calculate the average vertical profile of relative humidity
def calculate_average_profile(humidity, indices):
    return np.mean(humidity[indices, :], axis=0) if indices else None

# Plot all four seasons in one figure
fig, axs = plt.subplots(2, 2, figsize=(15, 12))

# Function to plot average relative humidity profiles for each season
def plot_season_profiles(ax, season, athens_time_indices, thessaloniki_time_indices, athens_humidity, athens_geopotential, thessaloniki_humidity, thessaloniki_geopotential):
    for time_label, indices in athens_time_indices.items():
        avg_profile = calculate_average_profile(athens_humidity, indices)
    ...
    for time_label, indices in thessaloniki_time_indices.items():
        avg_profile = calculate_average_profile(thessaloniki_humidity, indices)
        if avg_profile is not None:
    ...
    ax.set_title(f'Average Vertical Profile of Relative Humidity in {season}')
    ax.legend()
    ax.set_xlim(0, 100)  # Zoom in on x-axis to show relative humidity range (0 to 100%)
    ax.set_ylim(0, 3)  # Zoom in on y-axis to show height range (0 to 3 km)

# Plot for Winter
plot_season_profiles(axs[0, 0], 'Winter', athens_winter_time_indices, thessaloniki_winter_time_indices, athens_humidity, athens_geopotential, thessaloniki_humidity, thessaloniki_geopotential)

# Plot for Spring
plot_season_profiles(axs[0, 1], 'Spring', athens_spring_time_indices, thessaloniki_spring_time_indices, athens_humidity, athens_geopotential, thessaloniki_humidity, thessaloniki_geopotential)

# Plot for Summer
plot_season_profiles(axs[1, 0], 'Summer', athens_summer_time_indices, thessaloniki_summer_time_indices, athens_humidity, athens_geopotential, thessaloniki_humidity, thessaloniki_geopotential)

# Plot for Autumn
plot_season_profiles(axs[1, 1], 'Autumn', athens_autumn_time_indices, thessaloniki_autumn_time_indices, athens_humidity, athens_geopotential, thessaloniki_humidity, thessaloniki_geopotential)
